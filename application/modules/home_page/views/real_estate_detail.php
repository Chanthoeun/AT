...
<section class="row">
    <div class="col-sm-9 col-md-9 col-lg-9" itemscope itemtype="http://schema.org/Article">
        <h3 class="page-header" itemprop="name">
            <?php echo $real_estate->title; ?>
        </h3>
        
        <div class="release">
            <p class="pull-left"><?php echo lang('home_product_posted_at').' <strong>'.date('l, d F Y', time($real_estate->created_at)).'</strong>'; ?></p>
            <p class="pull-right"><span class="addthis_sharing_toolbox pull-right"></span></p>
        </div>

        <?php if($real_estate->file != FALSE): ?>
        <figure>
            <img src="<?php echo base_url(get_uploaded_file($real_estate->file)); ?>" alt="<?php echo $real_estate->title; ?>" />
        </figure>
        <?php endif; ?>
        ...
        <div role="tabpanel">
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="detail"><?php echo $real_estate->description; ?></div>
                <div role="tabpanel" class="tab-pane" id="photo">
                    <?php 
                        if(isset($medias) && $medias != FALSE)
                        {
                            foreach ($medias as $media){
                                if($real_estate->file != $media->file)
                                {
