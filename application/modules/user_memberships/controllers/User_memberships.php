 ...
 * @author Chanthoeun
 */
class User_memberships extends Admin_Controller {
    ...
    public function index()
    {
        ...
        redirect('memberships', 'refresh');
    }

    public function get_index($cid)
    {
        parent::check_login();
        $membership = Modules::run('memberships/get', $cid);
        ...
        $layout_property['breadcrumb'] = array('memberships/'.check_member_type($this->data['membership']->id) => $this->lang->line('index_membership_heading'), 'memberships/view/'.$membership->id => $membership->name, $title);
        
        $layout_property['content']  = 'index';
        $layout_property['sidebar'] = 'memberships/sidebar';
    ...
    {
        parent::check_login();
        $membership = Modules::run('memberships/get',$mid);
        $this->data['membership'] = $membership;        
        
        $user = $this->get_with_user(array('user_membership.id' => $id));
        
        $this->data['hidden_data']  = array(
        ...
        $layout_property['breadcrumb'] = array('memberships/'.check_member_type($this->data['membership']->id) => $this->lang->line('index_membership_heading'), 'memberships/view/'.$membership->id => $membership->name, 'user_memberships/'.$membership->id => $this->lang->line('index_user_membership_heading'), $title);
    ...
    {
        parent::check_login();
        $cid = $this->input->post('membership_id');
        ...
        if($this->user_membership->validate($data))
        {
            if($this->ion_auth->update($user_membership->user_id, $data))
            {
                ...
                redirect('user_memberships/'.$cid, 'refresh');
            }
            else
            {
                $this->session->set_flashdata('message', $this->lang->line('form_user_membership_report_error'));
                redirect_form_validation(validation_errors(), $this->input->post(), 'user_memberships/edit/'.$cid.'/'.$id);
            }
        }
        else
        {
